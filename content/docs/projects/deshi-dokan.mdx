...
description: >-
  হাতে-কলমে বানাবো "দেশি দোকান" — একটি সম্পূর্ণ ফাংশনাল ই-কমার্স স্টোর। থাকছে Redux Toolkit দিয়ে Cart State Management, RTK Query দিয়ে Data Fetching ও Mutation (Axios ছাড়াই), json-server দিয়ে নিজের Mock Backend, আধুনিক Tailwind CSS UI, আর React Router দিয়ে Multi-page navigation। যিনি জীবনে কখনো Redux ব্যবহার করেননি, তিনিও এই গাইড অনুসরণ করে সম্পূর্ণ প্রজেক্টটা বানাতে পারবেন।
---

# দেশি দোকান — একটি সম্পূর্ণ হাতে-কলমে প্রজেক্ট (Redux Toolkit + RTK Query)
...
> এই টিউটোরিয়ালটা সম্পূর্ণ Redux-বিগিনারদের জন্য লেখা। জীবনে কখনো Redux ব্যবহার না করে থাকলেও সমস্যা নেই — প্রতিটা concept শুরু থেকে ব্যাখ্যা করা হয়েছে, কোনো prior Redux knowledge ধরে নেওয়া হয়নি।
...
### ২.৩ Redux Toolkit, react-redux, react-router ইনস্টল করা
...
>
> লক্ষ্য করুন — উপরে axios ইনস্টল করিনি। **RTK Query আলাদা কোনো প্যাকেজ না** — এটা `@reduxjs/toolkit`-এর ভেতরেই বিল্ট-ইন থাকে, `@reduxjs/toolkit/query/react` থেকে import হবে (সেকশন ৮-এ দেখবেন)। তাই ডাটা fetch করার জন্য axios বা fetch আলাদা করে ইনস্টল করার দরকার নেই।

### ২.৪ Mock Backend — json-server ইনস্টল করা
...
**concurrently** কেন লাগবে? — আমাদের দুটো সার্ভার একসাথে চালাতে হবে: Vite dev server (frontend) আর json-server (mock backend)। `concurrently` দিয়ে একটাই কমান্ডে দুটোই চালানো যাবে (সেকশন ২.৭-এ দেখবেন)।
...
### ২.৭ `package.json`-এ Script যোগ করা
...
## ৫. Store তৈরি করা এবং App-কে Provider দিয়ে wrap করা
...
> `productsApi` ফাইলটা আমরা এখনো বানাইনি — সেকশন ৮-এ বানাবো। এখন শুধু বুঝে নিন, `configureStore`-এ **দুই ধরনের reducer** যোগ হয়েছে — একটা আমাদের নিজের `cartReducer`, আরেকটা RTK Query-র নিজস্ব `productsApi.reducer` (এটা RTK Query-র caching data রাখে)। `middleware`-এ `productsApi.middleware` যোগ না করলে caching, auto-refetch, polling-এর মতো RTK Query-র ফিচারগুলো কাজ করবে না — এটা must।
...
## ৬. RTK Query কী? Axios + `useEffect`-এর থেকে কীভাবে আলাদা?
...
_Table 6.1: Manual Axios + useEffect vs RTK Query_
...
## ৭. Mock Backend Endpoints — Reference

_Table 7.1: আমাদের json-server mock backend-এর সব endpoint_
...
## ৮. Products API Slice — RTK Query-র মূল অংশ
...
- **Auto-generated hooks** — `getProducts` এন্ডপয়েন্ট থেকে自動ভাবে `useGetProductsQuery` হুক তৈরি হয়ে যায় (নাম প্যাটার্ন: `use` + PascalCase এন্ডপয়েন্ট নাম + `Query`/`Mutation`)। এই হুকগুলোই component-এ ব্যবহার করব — নিজে কিছু বানাতে হবে না।

### ৮.২ `providesTags` আর `invalidatesTags` — RTK Query-র আসল জাদু
...
`Navbar` সব route-এর বাইরে বসানো হয়েছে, যাতে প্রতিটা পেজেই দেখা যায়। এবার একে একে প্রতিটা কম্পোনেন্ট আর পেজ বানাবো।
    ...
    return (
      <div className="text-center mt-16">
        <p className="text-gray-500">
          কার্ট খালি — চেকআউট করার আগে প্রোডাক্ট যোগ করুন।
        </p>
        <Link to="/" className="text-emerald-700 underline">
          শপিং শুরু করুন
        </Link>
...
export default function OrdersPage() {
  ...
  if (isError) {
    return (
      <p className="text-center text-red-600 mt-10">অর্ডার লোড করা যায়নি।</p>
    );
  }
...
এই exercise আপনাকে independently আরেকটা সম্পূর্ণ Redux Toolkit slice বানানোর অনুশীলন দেবে — ঠিক Deshi Book প্রজেক্টের "own feed update" exercise-এর মতোই, কিন্তু এবার axios/reducer-এর বদলে Redux Toolkit দিয়ে।
...
| Environment variable | `.env`-এ `VITE_API_BASE_URL`           | প্রতিটা environment (dev/staging/prod)-এর জন্য আলাদা `.env` ফাইল বা deployment platform-এর env variable ব্যবহার করুন                      |
...
- **React Router:** Dynamic route params (`useParams`), `useNavigate`, Multi-page navigation।

এই pattern গুলো — Redux Toolkit slice + RTK Query API slice + tag-based invalidation — যেকোনো production React বা Next.js প্রজেক্টে সরাসরি ব্যবহার করা যাবে। শুধু `baseUrl` আর endpoint-গুলো নিজের real backend অনুযায়ী বদলে নিলেই হবে।
