...
> **মনে রেখো:** এই json-server শুধু Part 1-এর বেসিক প্র্যাকটিসের জন্য। Part 2-এর আসল Connectify project-এ উপরে বলা `rnext` backend-ই ব্যবহার হবে।
...
প্রজেক্ট ফোল্ডারে একটা `db.json` ফাইল বানাও, ভেতরে কিছু ডামি ডেটা রাখো:

```json
{
  "posts": [
    { "id": "1", "title": "প্রথম পোস্ট", "body": "json-server দিয়ে তৈরি প্রথম পোস্ট।" },
    { "id": "2", "title": "দ্বিতীয় পোস্ট", "body": "Axios দিয়ে এই ডেটা fetch করা হবে।" }
  ],
...
সার্ভার চালু হলে টার্মিনালে `http://localhost:8000` ঠিকানা দেখাবে। ব্রাউজারে গিয়ে ওই লিংকে চেক করো — `posts` আর `comments` ডেটা JSON আকারে দেখাচ্ছে কিনা।

### ২.৪ json-server-এর Routes চেনা

json-server তোমার `db.json`-এর প্রতিটা key-কে automatically একটা REST resource বানিয়ে দেয়। সব HTTP method (GET, POST, PUT, PATCH, DELETE) সাপোর্ট করে, আর `http` বা `https` — দুটো দিয়েই request পাঠানো যায়:
...
ধরো, আগে তোমার React কম্পোনেন্টে একটা লোকাল `db.js` ফাইল থেকে ডেটা import করে দেখাতে। এখন সেটা বদলে আসল API (আমাদের json-server) থেকে আনতে হবে।

### ৩.১ State সেটআপ করা

সবার আগে ডেটা আর error রাখার জন্য state বানাও:
...
সার্ভার বন্ধ রেখে test করো — `error` দেখাবে। সার্ভার চালু রেখে test করো — পোস্ট লিস্ট দেখাবে। দুই ধরনের error ঠিক কীভাবে আলাদা করে ধরতে হয়, সেটা সেকশন ৪-এ বিস্তারিত দেখবো।
...
`handleAddPost`-কে `async` করা হয়েছে, কারণ ভেতরে `await axios.post(...)` ব্যবহার হচ্ছে। POST request-এ দ্বিতীয় argument হিসেবে যে ডেটা পাঠাতে চাও, সেটা যায়।
...
**PATCH বনাম PUT — পার্থক্যটা মনে রাখো:**
...
const handleDeletePost = async (postId) => {
  const confirmed = window.confirm("পোস্টটা সত্যিই ডিলিট করতে চাও?");
  if (!confirmed) return;
...
লক্ষ্য করো, template literal দিয়ে URL-এর শেষে নির্দিষ্ট id জোড়া দেওয়া হচ্ছে — DELETE আর PATCH দুটোতেই একই প্যাটার্ন।
...
আলাদা একটা ফাইলে (`api.js`) একবারই instance তৈরি করে ফেলো:
...
> **মনে রেখো:** এখনই `Authorization` header যোগ করো না। Token dynamically যোগ করার কাজটা **Interceptor** দিয়ে করবো — একদম পরের সেকশনে দেখো।
